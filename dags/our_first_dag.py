...
def daily_etl_pipeline():
    ...
    def extract_market_data(market: str):
        """ Simulate extracting market data for popular companies. This task mimics pulling live stock prices or API data."""
        ...
        print(f"[EXTRACT] Market data for {market} successfully generated at {raw_path}")
        return raw_path
    
    @task
    def transform_market_data(raw_file: str):
        """ Clean and analyze extracted market data. This task stimulates transforming raw
        stock data to identify the top gainers and losers of the day."""
        ...
        # print(f"[TRANSFORM] Top gainers save to {gainers_path}")
        # print(f"[TRANSFORM] Top losers save to {losers_path}")
    ...
    transformed = transform_market_data.expand(raw_file=raw_files)
    loaded = load_to_mysql.expand(transformed_file=transformed)

    # Preview MySQL after all loads complete
